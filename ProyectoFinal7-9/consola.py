...
# Importamos todo lo necesario de nuestro modelo
from modelo import (
...
# ==========================================
ARBOL = None        # La raíz del árbol
INDICE_TRIE = None  # El índice para búsquedas rápidas
RUTA_ACTUAL = "/"   # Texto de la ruta actual (ej: /documentos)
NODO_ACTUAL = None  # El objeto nodo donde estamos parados

# ==========================================
# UTILIDADES DE RUTA Y CARGA
# ==========================================
...
def inicializar_sistema():
    ...
    # 3. Construir el índice Trie (Día 5-6)
    print(" > Construyendo índice de búsqueda...")
    INDICE_TRIE = Trie()
    construir_trie_inicial(ARBOL, INDICE_TRIE)
    
    # 4. Iniciar en la raíz
    NODO_ACTUAL = ARBOL

def resolver_ruta_absoluta(argumento_ruta):
    """Convierte una ruta relativa o absoluta en absoluta."""
    if not argumento_ruta:
        return RUTA_ACTUAL
        
    if argumento_ruta.startswith("/"):
        return argumento_ruta # Ya es absoluta
    
    # Es relativa, concatenar con la actual
    if RUTA_ACTUAL == "/":
...
def cmd_cd(nombre_carpeta):
    ...
    if nombre_carpeta == "..":
        # Volver al padre (simple implementación: volver a root o recalcular string)
        # Para simplificar este proyecto escolar, '..' nos lleva a root por ahora
        # o implementamos búsqueda del padre real si tenemos tiempo.
...
def cmd_crear(nombre, tipo, contenido=""):
    """Lógica compartida para mkdir y touch."""
    # 1. Verificar duplicados en el nodo actual
    if NODO_ACTUAL.buscar_hijo_por_nombre(nombre):
        print(f"Error: Ya existe '{nombre}' aquí.")
        return
...
def cmd_rm(nombre):
    ...
    # Usamos la función poderosa de modelo.py que ya maneja el Trie
...
def iniciar_consola():
    ...
    while True:
        try:
            ...
            args = partes[1:] # Lista de argumentos

            if comando == "exit":
                guardar_arbol_a_json(ARBOL, "arbol_persistente.json")
                print("Guardando cambios... ¡Adiós!")
                break
